...
class ProductView(View):
 def get(self,request):
  ...
  return render(request,'app/home.html',{'topwears':topwears,
        ...
        'shoes':shoes,'trend':trend,'dday':dday})
...
def shoes(request,data=None):
  if data==None:
    ...
    shoes=Product.objects.filter(category='SH').filter(discounted_price__lt=500)
  elif data=='above':
    shoes=Product.objects.filter(category='SH').filter(discounted_price__gt=500)
